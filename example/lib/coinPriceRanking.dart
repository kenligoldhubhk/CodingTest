...
class _CoinPriceRankingPageState extends State<CoinPriceRankingPage> {
  ...
  /// called in initState twice (can use Rx timer)
  /// TODO fade in
      ...
      for (var i = 0; i < coinPriceList.coinPriceList.length; i++) {
        ...
        widgetList.add(
          Row(children: <Widget>[
            ...
            //
            Text( coinPrice.currentPrice.toString()),
            // TODO +/- % icon
            Builder(
              builder: (_) {
                double value = coinPrice.priceChangePercentage24h;
                if (value > 0) {
                  return Row(children: [
                    Container(
                      height: 40,
                      width: 20,
                      child: ImageIcon(
                        AssetImage("images/priceUpIcon.png"),
                      ),
                    ),
                    // TODO color green
                    Text(value.toString(),style:TextStyle(color:FXColor.lightGray)),
                  ]);
                } else if (value < 0) {
                  return Row(children: [
                    Container(
                      ...
                      width: 20,
                      child: ImageIcon(
                        AssetImage("images/priceDownIcon.png"),
                      ),
                    ),
                    // TODO color red
                    Text(value.toString(),style:TextStyle(color:FXColor.lightGray)),
